(ns cashier-ticketing.actor
  "CashierTicketingActor — the ISCO-08 5230 independent cashier-and-
  ticketing actor as a `langgraph.graph/state-graph` (per
  ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  transact/reconcile operation request (intake → advise → govern →
  ...
  The unconditional invariant: the CashierAdvisor can never directly
  commit a record or dispatch a robot action the
  CashierTicketingGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [cashier-ticketing.advisor :as advisor]
            [cashier-ticketing.governor :as governor]
            [cashier-ticketing.store :as store]))

(defn build-graph
  "Build a compiled CashierTicketingActor graph. `store` implements
  `cashier-ticketing.store/Store`. `advisor` implements
  `cashier-ticketing.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:shift-id (:shift-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
